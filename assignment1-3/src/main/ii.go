...
func mapF(document string, value string) (res []mapreduce.KeyValue) {
	...
	for endIndex := 0; endIndex < len(runes); endIndex++ {
		if unicode.IsLetter(runes[endIndex]) {
			...
			if isCounting {
				isCounting = false
				result = append(result, mapreduce.KeyValue{string(runes[startIndex:endIndex]), document})
				startIndex = -1
			} else {
				continue
			}
...
func reduceF(key string, values []string) string {
	...
	keys := make([]string, 0, len(occurences))
	for k := range occurences {
		keys = append(keys, k)
	}
	sort.Strings(keys)
	return fmt.Sprintf("%d %s", len(occurences), strings.Join(keys, ","))
}
